...
# Remove spaces from separators
separators=${separators//[[:space:]]/}

# Escape certain characters in the separators
separators=$(sed 's/[][\+\.\*\?\^\$]/\\&/g' <<< "$separators")

# Build the sed command with multiple -e options
sedCommandMyFile="sed "
sedCommandOldFile="sed "
sedCommandYourFile="sed "
for separator in $(echo $separators | fold -w1); do
    sedScript="s/$separator/\n\$\$\$\$\$\$\$$separator\n\$\$\$\$\$\$\$/g"
    sedCommandMyFile+="-e '${sedScript}' "
    sedCommandOldFile+="-e '${sedScript}' "
    sedCommandYourFile+="-e '${sedScript}' "
done

# Apply the sed command to the input files
sedCommandMyFile+="$myFile > ${myFile}_temp${fileExt}"
sedCommandOldFile+="$oldFile > ${oldFile}_temp${fileExt}"
sedCommandYourFile+="$yourFile > ${yourFile}_temp${fileExt}"
